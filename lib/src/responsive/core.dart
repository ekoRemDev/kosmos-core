...
T getResponsiveValue<T>(BuildContext context, {required T defaultValue, T? desktop, T? tablet, T? phone}) => ResponsiveValue<T>(context, defaultValue: defaultValue, valueWhen: [
...
class ResponsiveBuilder extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (ResponsiveValue(context, defaultValue: false, valueWhen: [
      const Condition.smallerThan(name: PHONE, value: true),
    ]).value!) {
      return phoneChild ?? tabletChild ?? child;
    } else if (ResponsiveValue(context, defaultValue: false, valueWhen: [
